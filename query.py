...
# psp
psp_query = f"""select * from gna_database.psp_report_all_india where date = '{date}'  and regions='TOTAL'  allow filtering"""
...
frequency_query = f"""select * from gna_database.frequency_psp where id = '{date}' allow filtering"""
frequency_previous_query = f"""select * from gna_database.frequency_psp where id = '{lastYeardate}' allow filtering"""


# Fossil Fuels 
source_wise_query = f"""select * from gna_database.sourcewise_generation where date = '{date}' and region='All India' allow filtering"""
source_wise_previous_query = f"""select * from gna_database.sourcewise_generation where date = '{lastYeardate}' and region='All India' allow filtering"""
psp_report_query = f"""select * from gna_database.psp_report_all_india where date = '{date}' allow filtering"""
